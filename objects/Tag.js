function Tag(id, title, node) {
	...
	this.element = $('<div>').prependTo('#tags' + this.node.id).addClass('tag').attr('id', 'tag' + this.id); //set the id to the node's id to be able to select tags easily
	this.titleElement = $('<h5>').appendTo(this.element).text(this.title);
	$('<img>').appendTo(this.element).attr({src: 'images/icons/x.svg', id: 'tag_x_' + this.id}).addClass('iconic-sm tag_x');
	var id = this.id;
